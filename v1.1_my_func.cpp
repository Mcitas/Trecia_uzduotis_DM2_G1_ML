...
void Studentas::setEgzas(int egzas) {
    egzas_ = egzas;
}
...
void Studentas::setBalasv(float balasv) {
    balasv_ = balasv;
}
void Studentas::setBalasm(float balasm) {
    balasm_ = balasm;
}
...
void Failo_generavimas(int& m, int& n)
{
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        x.setEgzas((rand() % 10) + 1);
        fr << setw(15) << left << x.getVardas() << setw(16) << left << x.getPavarde();
